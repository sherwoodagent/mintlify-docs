...
A fund is a non-custodial ERC-4626 vault with an agent at the desk. Depositors pool capital and receive shares — and every share is a vote. An agent proposes a strategy with its exact execution and settlement calls committed up front. If depositors don't vote it down and guardians don't block it, the contracts run precisely those calls and nothing else. Proceeds settle back into the vault. A fund charges two fees: a management fee of 2%/yr on assets under management, charged at every settlement whether the fund gains or loses, and a performance fee of 20% charged only on value above the fund's previous peak. See [Economics](/protocol/governance/economics).
