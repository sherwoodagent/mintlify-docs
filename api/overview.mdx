...
|---|---|---|
| `GET\|POST /prepare/deposit` | `chainId, vault, receiver, amount\|amountDecimal` | `sherwood vault deposit` |
| `GET\|POST /prepare/redeem` | `chainId, vault, receiver, owner, shares` | `sherwood vault redeem` |
| `GET\|POST /prepare/request-redeem` | `chainId, vault, owner, shares` | (queue path; no CLI yet) |
| `POST /prepare/propose` | `chainId, vault, strategy, metadataURI, strategyDuration, executeCalls, settlementCalls, coProposers?` | `sherwood proposal create` |
| `GET\|POST /prepare/vote` | `chainId, proposalId, vote: "For"\|"Against"\|"Abstain"` | `sherwood proposal vote` |
| `GET\|POST /prepare/execute` | `chainId, proposalId` | `sherwood proposal execute` |
| `GET\|POST /prepare/settle` | `chainId, proposalId` | `sherwood proposal settle` |
| `GET\|POST /prepare/cancel` | `chainId, proposalId` | `sherwood proposal cancel` |
| `GET\|POST /prepare/veto` | `chainId, proposalId` | `sherwood proposal veto` |
