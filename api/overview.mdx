...
**Both verbs supported.** Every `/prepare/*` route accepts `GET` (query string) and `POST` (JSON body) and returns identical calldata — except `/prepare/propose`, `/prepare/emergency-settle` and `/prepare/strategy-deploy`, which are POST-only: their nested `executeCalls[] / settlementCalls[] / coProposers[]` arrays don't query-encode cleanly, and `initData` can outrun URL length limits. GET is the easier one-liner; POST is preserved for backward compatibility and complex payloads.
...
| `GET\|POST /prepare/vote-on-proposal` | `chainId, vault, proposalId, vote: "Approve"\|"Block", lockWood` | `sherwood proposal review-vote` |
...
| `GET\|POST /prepare/create-fund` | `chainId, creatorAgentId, metadataURI, asset, name, symbol, openDeposits, subdomain` | `sherwood vault create` |
| `GET\|POST /prepare/approve-depositor` | `chainId, vault, depositor` | `sherwood vault approve-depositor` |
| `GET\|POST /prepare/register-agent` | `chainId, vault, agentAddress, agentId` | `sherwood vault add` |
| `GET\|POST /prepare/approve-agent` | `chainId, vault\|subdomain, agentAddress, agentId` | `sherwood vault approve` |
| `GET\|POST /prepare/join` | `chainId, vault\|subdomain, agentId, message` | `sherwood vault join` |
...
**`/prepare/vote-on-proposal` requires `lockWood`.** It is the WOOD the guardian declares as coverage behind an Approve, passed as a **decimal string of base units** (`"1000000000000000000000"`) so a JSON number cannot lose precision on an 18-decimal amount. There is no default and no overload without it. `"0"` is legal and is what a Block passes. See [Coverage and underwriting](/protocol/governance/coverage) for how the ledger clamps it and why too small a lock reverts.
...
| `GET /governor?chain=46630` | Live governor parameters (voting period, veto bps) plus the protocol's fee splits. **v1 break:** the flat `protocolFeeBps` / `guardianFeeBps` are gone, replaced by the nested `mgmtSplit` / `perfSplit` objects. |
...
| `GET /proposals?chain=46630&limit=25&state=Pending&vault=0x...` | List recent proposals (descending), with optional state + vault filters. **v1 break:** `votesFor` / `votesAbstain` are gone — voting is optimistic and only AGAINST weight is tallied — and `votableSupply` (the electorate snapshotted at propose) is new. |
| `GET /proposals/:id?chain=46630` | Single proposal: against-vote tally, `votableSupply`, state, strategy, fee, execute/review/settle deadlines. Returns 404 on unknown id. Same field change as `/proposals`. |
