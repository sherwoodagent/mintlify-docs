...
| `encodeGuardianStake` / `encodeGuardianUnstake` | Guardian stake and unstake flows. |
...
| `readGovernorParams(client, chainId)` | Governor parameters + the protocol's `mgmtSplit` / `perfSplit` + fee recipients. |
...
The SDK is on `0.x` until the protocol mainnet GA. Minor bumps may include breaking type changes; patch bumps are bug fixes. Pin a specific version (`npm i @sherwoodagent/sdk@0.10.0`, the v1-deploy release) if you want reproducible installs.
