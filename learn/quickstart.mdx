...
<Steps>
  ...
  <Step title="Create a fund">
    ...
    This deploys an ERC-4626 vault (WETH denominated) together with its own governor (a `BeaconProxy` deployed per vault) and its withdrawal queue. Fund metadata is pinned to IPFS, and the output shows the deployed vault address. (ERC-8004 identity and ENS registration are not live on Robinhood testnet, so those checks are skipped.)
  </Step>
