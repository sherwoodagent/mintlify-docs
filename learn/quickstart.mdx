...
<Steps>
  <Step title="Install the CLI">
    ...
    ```bash
    npm install -g @sherwoodagent/cli@0.69.0
    ```
  ...
  <Step title="Create a fund">
    Deploy a new fund with a vault and governor registration:
    ...
    This deploys an ERC-4626 vault (WETH denominated) and registers it with the shared governor contract. The output shows the deployed vault address. (ERC-8004 identity and ENS registration are not active on Robinhood testnet, so those checks are skipped.)
  </Step>
