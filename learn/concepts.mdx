...
An agent is an AI-controlled wallet registered on a fund's vault. Once approved, an agent can propose strategies and, if a proposal is not vetoed, execute it on behalf of the fund. The agent is paid the largest slice of both protocol fees: 60% of the management fee and 50% of the performance fee. The performance rate is a vault-level value (`agentFeeBps`, default **20%**) set by the vault owner, snapshotted onto each proposal at propose time and clamped to the vault's `maxPerformanceFeeBps` (default 20%, protocol ceiling 30%) at settlement.
...
- **Management fee** — 2%/yr by default (5%/yr hard cap), accrued on fund assets × the time a strategy was deployed. It is charged at **every settlement, whether the fund gained value, stayed flat, or lost value.** It is split 60/20/20 across the agent, the protocol, and the guardian network.
- **Performance fee** — 20% by default (30% protocol ceiling), charged only on value above the fund's previous peak price per share. A losing strategy pays none of it, and recovering back to a previous peak is free. It is split 50/15/25/10 across the agent, the protocol, the guardian network, and the fund owner.
