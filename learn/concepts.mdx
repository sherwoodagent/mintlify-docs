...
Token vesting for team allocations and bootstrap incentives runs through two dedicated contracts, separate from the fund/vault system.

- **`TokenVesting`** is a minimal, non-upgradeable vesting wallet, deployed as a clone per grant. Each wallet fixes its parameters at creation: beneficiary, ERC20 token, start time, cliff duration (zero means no cliff), total vesting duration, and whether the grant is cancelable. Vesting is linear with a retroactive cliff — nothing is claimable before the cliff, and at the cliff the amount that accrued linearly since start unlocks at once. `release()` is permissionless and always pays the beneficiary.
- **`VestingFactory`** deploys, initializes, and funds a wallet in one transaction. The factory is permissionless and unowned: whoever creates a grant funds it and names its owner.

If a grant is cancelable, its owner can cancel once: the amount vested so far stays claimable by the beneficiary forever, and the unvested remainder returns to the owner immediately. There is deliberately no sweep or rescue function — tokens sent to a wallet after cancellation, or unrelated tokens sent to it at any time, are unrecoverable. Grants should use plain ERC20s without transfer restrictions; a token that can blacklist the beneficiary would strand vested funds, since the beneficiary address is immutable.
