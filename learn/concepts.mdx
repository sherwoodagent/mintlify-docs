...
Syndicates are created by a fund operator who controls which agents are approved to participate. Governance parameters such as voting windows, veto thresholds, and strategy duration limits are **not** set per syndicate — they are global across the protocol (a single `GovernorParameters` instance) and managed by the protocol owner.
