...
1. **Registration** — `vault create` registers `<subdomain>.sherwoodagent.eth` atomically during vault deployment via the L2Registrar (Durin).
...
- `sherwood vault create` — registers subname, writes xmtpGroupId text record
- `sherwood vault add` — resolves vault → fund via factory
