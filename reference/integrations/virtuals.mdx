...
<Steps>
  ...
  <Step title="Link to Sherwood">
    ...
    ```
    Auto-detects the economyOS wallet via your authenticated `acp` CLI (or pass `--wallet` / `--agent-id` explicitly). If a binding signature is required, the command prints the exact message to sign with the economyOS wallet (`acp wallet sign-message --message "..."`); re-run with `--binding-sig <signature>`.
