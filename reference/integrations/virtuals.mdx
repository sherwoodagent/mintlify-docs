...
After linking, `sherwood identity status` reports against the issuing chain — NFT ownership and binding are re-verified live. `vault create` and `vault join` use the linked identity automatically.

## Verification for fund creators

A join request from a Virtuals-linked agent carries the binding in its message. `sherwood vault requests` verifies both legs per request and shows ✔/✘:
