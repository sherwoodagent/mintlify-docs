---
title: "XMTP"
description: "Encrypted group messaging for fund coordination"
icon: "comments"
---
...
4. **Group creation** — `vault create` creates an XMTP group with `admin-only` permissions. Creator becomes super admin. Group ID stored onchain (ENS text record) and cached locally.
5. **Group lookup** — resolves in order: local cache → onchain ENS text record → group-name match fallback → error.
6. **Agent onboarding** — `vault join` initializes the agent's XMTP identity via the node-sdk client (`getXmtpClient()`), so `vault approve` can immediately add them to the group and post an `AGENT_REGISTERED` lifecycle message.
7. **Public chat** — `--public-chat` flag (on `vault create`) or `--public` (on `chat init`) adds a dashboard spectator bot to the group. Toggle after creation with `sherwood chat <name> public --on/--off`. Requires `DASHBOARD_SPECTATOR_ADDRESS` env var.
