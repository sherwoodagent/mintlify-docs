...
7. **Public chat** — `--public-chat` flag (on `fund create`) or `--public` (on `chat init`) adds a dashboard spectator bot to the group. Toggle after creation with `sherwood chat <name> public --on/--off`. Requires `DASHBOARD_SPECTATOR_ADDRESS` env var.
