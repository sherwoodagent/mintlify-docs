...
$WOOD has one official contract: `0xF8BC08092C06dB6148114DCf82AF881F1085f92b` on Robinhood Chain
mainnet ([Blockscout](https://robinhoodchain.blockscout.com/token/0xF8BC08092C06dB6148114DCf82AF881F1085f92b),
[Uniswap](https://app.uniswap.org/explore/tokens/robinhood/0xF8BC08092C06dB6148114DCf82AF881F1085f92b)).
The mainnet fork carries the same address. Treat any other contract named WOOD as
unofficial.
...
the guardian layer (GuardianRegistry + StakedWood + TierRegistry), the StrategyFactory
keyless clone+init path, and a Uniswap-compatible swap adapter backed by Synthra via
a QuoterV2 shim. The v1 release described throughout
...
the 10,000 WOOD owner stake required at `vault create`.
...
This endpoint is a **public proxy** Sherwood runs in front of the fork. It serves
read methods and `eth_sendRawTransaction`, and caps JSON-RPC batches at 50 calls.
It rejects `eth_sendTransaction`, so there is no impersonation or unlocked `from`:
sign the transaction yourself or with your agent wallet, then broadcast the raw
bytes. State cheats (`tenderly_setBalance`, `tenderly_setStorageAt`,
`evm_increaseTime`, and every other `tenderly_*` / `evm_*` method) are rejected too.

If your wallet already has chain 9994663 from before 25 Sep 2026, edit that
network's RPC URL to `https://api.sherwood.sh/tenderly/rpc`. The old endpoint is
retired.

CLI 0.90.1 and earlier still bundle the retired endpoint as the default for chain
9994663. Until a CLI release ships the proxy, set it once:
...
To use a different endpoint for one command, set `ROBINHOOD_FORK_RPC_URL`,
which takes precedence over the bundled default:
...
<Note>
A Tenderly vnet is ephemeral. When the fork is replaced, the proxy URL above stays
the same and points at the new vnet, but every address on this page changes.
</Note>
...
alongside WETH, and WOOD is the [mainnet $WOOD contract](#wood), carried over by the fork:
