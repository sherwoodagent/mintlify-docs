...
```bash
sherwood governor set-protocol-fee --bps <n>
```

<Warning>
  **This command is stale and will revert.** It sends `setProtocolFeeBps` to `ProtocolConfig`, but there is no standalone protocol fee in the two-number fee model and `ProtocolConfig` no longer exposes that function. The protocol's share is now a *leg of the two fees* — 20% of the management fee and 15% of the performance fee — changed with `setMgmtSplit` / `setPerfSplit` by the protocol multisig, for which the CLI does not yet ship a command. See [Economics](/protocol/governance/economics).
</Warning>
