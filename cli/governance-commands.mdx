...
- Defaults to **20% (2000 bps)** at vault creation — the headline rate. An explicit `0` is legal.
- Capped at **30% (3000 bps)** by the vault, and additionally clamped to the governor's `maxPerformanceFeeBps` (default 2000 bps) at settlement. Going above the 20% headline therefore takes two changes, not one.
