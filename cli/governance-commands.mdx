...
- Defaults to **20% (2000 bps)** at vault creation — the headline rate.
- Capped at **25% (2500 bps)** by the vault (`MAX_AGENT_FEE_BPS`), and additionally clamped to the governor's `maxPerformanceFeeBps` at settlement (a factory-created vault starts that ceiling at 2000 bps).
...
<Note>
There is no "Registered Vaults" list any more — each governor serves exactly one vault. The protocol and guardian shares are not shown here either; they are legs of the splits on the shared `ProtocolConfig`, not per-vault governor parameters.
</Note>
...
There is no protocol-fee *rate*. The protocol and guardian shares are **split legs** of the management and performance fees, held on the protocol-wide `ProtocolConfig` (resolved via `factory.protocolConfig()`). Both commands take **no `--vault`** and are **protocol-owner only** (the protocol multisig). Each set of legs must sum to 10000 bps.

```bash
# management fee split — launch values 60 / 20 / 20
sherwood governor set-mgmt-split --agent 6000 --protocol 2000 --guardian 2000

# performance fee split — launch values 50 / 15 / 25 / 10
sherwood governor set-perf-split --agent 5000 --protocol 1500 --guardian 2500 --owner 1000
```

See [Economics](/protocol/governance/economics) for what each leg pays for.
