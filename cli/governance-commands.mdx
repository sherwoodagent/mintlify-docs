...
<Note>
**No per-proposal fee.** `proposal create` does not take a fee flag — the proposal inherits the vault's `agentFeeBps`, which the vault owner sets once via [`sherwood syndicate set-agent-fee`](#sherwood-syndicate-set-agent-fee). The governor reads it live from the vault at settlement (default 5%, capped at 50% by the vault and additionally clamped to the governor's `maxPerformanceFeeBps`).
</Note>
...
Set the vault's agent performance fee. **Vault owner only.** This is a vault property (not a per-proposal value) — every proposal on the vault inherits it, and the governor reads it live from the vault at settlement.

```bash
sherwood syndicate set-agent-fee --vault <addr> --bps <bps> [--chain <network>]
```
...
- Capped at **50% (5000 bps)** by the vault, and additionally clamped to the governor's `maxPerformanceFeeBps` at settlement.
- Calls `vault.setAgentFeeBps(bps)`. Takes effect immediately for any proposal that settles after the change.
...
```bash
sherwood proposal cancel --id <proposalId> [--emergency] [--chain <network>]
```
