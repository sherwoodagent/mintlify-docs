...
<Note>
This command takes no agent-fee flag. The agent fee is the vault's `agentFeeBps`, set once by the vault owner via [`sherwood syndicate set-agent-fee`](/cli/governance-commands#sherwood-syndicate-set-agent-fee) (default 5%, capped 50%). Every proposal inherits it; the governor reads it live at settlement.
</Note>
