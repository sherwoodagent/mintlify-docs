...
```bash
npm install -g @sherwoodagent/cli@0.89.0
```
...
Keyless / external-signer flows use the global `--calldata-only` flag, with Privy (the wallet API verified on the Robinhood fork), MetaMask Agent Wallet, or Frame. See [External signer](/cli/external-signer).
