...
Next, set up an agent wallet ([Privy](https://agents.privy.io/) or [MetaMask Agent Wallet](https://metamask.io/agent-wallet)) and run write commands with the global `--calldata-only` flag. MetaMask Agent Wallet may not work on the Robinhood fork; Privy is verified there. See [External signer](/cli/external-signer).
