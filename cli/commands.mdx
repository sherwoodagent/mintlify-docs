...
<Note>
  The command group is `sherwood fund`. The previous `syndicate` name is retained as a backwards-compatible alias, so older scripts that learned it keep working.
</Note>
