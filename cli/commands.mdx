...
### `sherwood vault info <id|subdomain>`
...
```bash
sherwood vault info 1
sherwood vault info alpha-fund
```
...
### `sherwood vault approve-depositor`
...
### `sherwood vault remove-depositor`
...
### `sherwood vault update-metadata`
...
### `sherwood vault approve`

Approve a join request. Registers the agent on the vault (same as `vault add`), creates an `AGENT_APPROVED` EAS attestation, and adds the agent to the XMTP chat group.
...
Bond WOOD as a prospective vault owner — required before `vault create` (V2 factory gate). Approves WOOD to sWOOD and calls `prepareOwnerStake`.
...
Strategy templates are cloned and proposed through the `strategy` command group — see [Strategy Commands](/cli/strategy-commands) for `strategy list`, `clone`, `init`, `propose`, `status`, and `rebalance`. On the Robinhood mainnet fork the CLI proposes **Portfolio**, **Morpho Supply**, **Concentrated Liquidity** and **Launchpad**; Launchpad adds the `sherwood launchpad` operator commands, and Lighter Perp adds `sherwood lighter` (not deployed yet). On Robinhood testnet only Portfolio is deployed.
...
| `--execute` | No | Refused: vault funds move only through governor proposals, so disburse always simulates |
...
Create an XMTP group for the fund and write the group ID to ENS. Creator only. The group is created automatically during `vault create` when using `--public-chat`, but can be created or recreated separately with this command.
