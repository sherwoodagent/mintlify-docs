...
The CLI targets **Robinhood testnet (chain 46630)** by default, so most users never
set a chain. The **Robinhood mainnet fork (chain 9994663)** is also available via
`--chain robinhood-fork`; its public RPC ships with the CLI, and
`ROBINHOOD_FORK_RPC_URL` overrides it. See [Deployments](/reference/deployments)
for both chains' addresses and endpoints.
