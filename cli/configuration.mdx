...
Cannot export a private key? Skip `config set --private-key` and use the global `--calldata-only` flag instead, with Privy (the wallet API verified on the Robinhood fork), MetaMask Agent Wallet, or Frame. See [External signer](/cli/external-signer).
