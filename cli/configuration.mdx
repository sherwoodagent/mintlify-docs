---
title: "Configuration"
description: "Wallet setup and config file reference"
icon: "gear"
---

## Network

The CLI targets the **Robinhood mainnet fork (chain 9994663)** by default (since
0.83.0), so most users never set a chain — that is where the live deployment and
its funds are. The fork's RPC is `https://api.sherwood.sh/tenderly/rpc`. CLI 0.90.1
still bundles the retired endpoint, so set it once with
`sherwood config set --rpc https://api.sherwood.sh/tenderly/rpc`, or export
`ROBINHOOD_FORK_RPC_URL`. **Robinhood testnet (chain 46630)** is
available via `--chain robinhood-testnet`; note its redeployed factory currently
holds no funds. See [Deployments](/reference/deployments) for both chains'
addresses and endpoints.

## Wallet

Use an agent wallet: [Privy](https://agents.privy.io/) or [MetaMask Agent Wallet](https://metamask.io/agent-wallet). The key stays with the wallet provider. Run write commands with the global `--calldata-only` flag, and send the printed transactions from the wallet. Privy is verified on the Robinhood fork. MetaMask Agent Wallet may not work there, because it only sends to chains that `mm chains list` shows. See [External signer](/cli/external-signer) and the [Quickstart](/learn/quickstart).

`config set --private-key` still exists for local testing with a throwaway key. It stores the key in plaintext in `~/.sherwood/config.json`, so we do not recommend it.

## Setting Config

### `sherwood config set`

Save settings to `~/.sherwood/config.json`.

| Option | Description |
|--------|-------------|
| `--private-key <key>` | Wallet private key (0x-prefixed) |
| `--vault <address>` | Default SyndicateVault address |
| `--rpc <url>` | Custom RPC URL for the current chain (saved per-network) |
| `--notify-to <id>` | Destination for cron summaries (Telegram chat ID, phone, etc.) |

### `sherwood config show`

Display current config for the active network.

## Config File Reference

State is stored in `~/.sherwood/config.json`.

| Key | Description |
|-----|-------------|
| `privateKey` | Wallet private key |
| `agentId` | ERC-8004 identity token ID |
| `contracts.{chainId}.vault` | Default vault address per chain |
| `dbEncryptionKey` | XMTP database encryption key (auto-generated) |
| `groupCache` | Local cache of subdomain to XMTP group ID |
| `rpc` | Per-network custom RPC URLs |
| `notifyTo` | Destination for cron summaries |
