...
graph TD
    ...
    F -->|binds owner stake on create| S["StakedWood / sWOOD (UUPS)"]
    G -->|guardian review hooks| R["GuardianRegistry (UUPS)"]
    R -->|reads vote weight, calls to slash| S
    V <-->|execute / settle| G
...
UUPS upgradeable contract (owned by the Sherwood protocol multisig). The **sole WOOD custodian** for the guardian layer — guardian stake, DPoS delegation, vault-owner bonds, vote checkpoints, and all slashing execution live here. Introduced by the sWOOD staking-split refactor, which lifted this machinery out of `GuardianRegistry` to reclaim EIP-170 bytecode headroom and give each contract a single job.

sWOOD follows the **vote-escrow** model (veCRV, Cosmos bonded stake): the staked position carries slashing liability and governance power, which must stay welded to one accountable identity. sWOOD is **not a token** — it exposes no ERC-20 transfer surface (`balanceOf`, `transfer`, `transferFrom`, `approve` do not exist) and no OZ `IVotes` views; it is non-transferable by construction. Governance weight is read through a bespoke checkpoint API — `getPastVotes(guardian, timestamp)`, `getPastTotalVotes(timestamp)`, `guardianStake`, `isActiveGuardian` — not an OZ `ERC20Votes` / `IVotes` surface.
...
- Vault-owner bond lifecycle (`prepareOwnerStake`, `cancelPreparedStake`, `bindOwnerStake` / `transferOwnerStakeSlot` — factory-only, `requestUnstakeOwner`, `claimUnstakeOwner`, `requiredOwnerBond` math)
- Share-based DPoS delegation (`delegateStake`, `requestUnstakeDelegation`, `cancelUnstakeDelegation`, `claimUnstakeDelegation`) + per-delegate commission (`setCommission`, raise-rate cap)
- `Trace224` vote checkpoints — `getPastVotes(guardian, ts)` returns own + delegated-inbound weight
- Slashing execution: `slashGuardians(pid, approvers[], slashBps)` and `slashOwnerBond(vault)` — both **registry-only** (`onlyRegistry` guard). Slashed WOOD is burned to `0x...dEaD` (CEI + pull-burn fallback via `flushBurn`)

**Share-factor delegator slashing.** Each delegate's pool tracks `poolTokens` / `poolShares`; per-delegator state is **shares**, not raw tokens. When a delegate-guardian is slashed, the delegated-in stake burns pro-rata across all their delegators via a single `poolTokens[delegate] -= slashAmount` write — O(1), no per-delegate delegator cap, no gas-DoS. This is the Cosmos SDK validator-shares model. `claimUnstakeDelegation` redeems shares at the current (possibly slashed) rate.

**`delegationEnabled` feature flag.** `bool delegationEnabled`, default **off**, owner-only `setDelegationEnabled(bool)`. When `false`, `delegateStake` reverts (`DelegationDisabled`) — no new delegations — but the three delegation-unstake paths stay **open** so existing delegators can always exit. Existing delegated stake keeps voting and stays slashable until withdrawn.

**Owner-set parameters** (uniform `ParameterChangeFinalized` event): `minGuardianStake`, `minOwnerStake`, `coolDownPeriod`, DPoS commission constants, `delegationEnabled`, and the graduated-slash bounds `minSlashBps` / `maxSlashBps` (must keep `minSlashBps ≤ maxSlashBps ≤ 10_000`).

**Set-once wiring:** `initialize(owner, wood, governor, factory)`; the registry pointer is set via a one-time owner-only `setRegistry(registry)` (resolves the sWOOD ↔ registry circular dependency). Deploy order: sWOOD → registry → `swood.setRegistry(registry)`.

### GuardianRegistry

UUPS upgradeable contract (owned by the Sherwood protocol multisig) managing the guardian **review / emergency / reward** lifecycle. After the sWOOD staking-split refactor it holds **zero WOOD** — all staking, delegation, owner bonds, and slashing execution moved to [StakedWood](#stakedwood-swood). The registry reads vote weight from sWOOD and calls sWOOD to slash. Interacts with `SyndicateGovernor` + `SyndicateFactory` via privileged hooks.

**Responsibilities:**
- Review windows (`openReview`, `voteOnProposal`, `resolveReview`, `cancelReview`) with cohort snapshot at `openReview()` time to close denominator-manipulation attacks
- Emergency-settle review windows (`openEmergencyReview` — governor-only, `voteBlockEmergencySettle`, `resolveEmergencyReview`)
- Multi-asset guardian-fee reward pools (`fundProposalGuardianPool`, `claimProposalReward`, `claimDelegatorProposalReward`) + W-1 escrow (`flushUnclaimedApproverFee`)
- `BlockerAttributed` Merkl events for off-chain WOOD epoch-reward attribution
- Computes the **graduated slash severity** at `resolveReview` and forwards it to `sWOOD.slashGuardians(...)` — see below
- Appeal path: `fundSlashAppealReserve` + `refundSlash` (owner-only, capped at 20% of reserve per epoch)
- Pause / unpause: `pause` (owner), `unpause` (owner, or permissionless after the 7-day `DEADMAN_UNPAUSE_DELAY`)

**Graduated, voted slash severity.** `voteOnProposal(proposalId, support, slashBps)` — a Block vote *carries* a proposed slash severity (`uint256 slashBps`); Approve / Abstain votes ignore the field. At `resolveReview`, if block quorum is reached, the registry computes the **stake-weighted median** of blockers' proposed severities (median, not mean — a single whale blocker cannot drag the verdict), clamps it to sWOOD's owner-set `[minSlashBps, maxSlashBps]` band, and passes the result into `sWOOD.slashGuardians`. This reduces own stake and delegated `poolTokens` by the same `slashBps`.

**Owner-set parameters:** `reviewPeriod` (24h default), `blockQuorumBps` (30% default), plus hard-coded constants `MIN_COHORT_STAKE_AT_OPEN`, `MAX_APPROVERS/BLOCKERS_PER_PROPOSAL`, `SWEEP_DELAY`, `LATE_VOTE_LOCKOUT_BPS`. Staking parameters (`minGuardianStake`, `minOwnerStake`, `coolDownPeriod`) moved to sWOOD.

**Set-once wiring** stamped at `initialize()`: `governor`, `factory`, `swood` (the registry holds no `wood` reference). No setters exist — rewiring requires a full registry redeploy under UUPS.

See [Guardian Review](/protocol/governance/guardian-review) for the full lifecycle and the design spec at `docs/superpowers/specs/2026-05-21-swood-staking-split-design.md` in the main repo.
...
5. **WOOD is assumed to be a fixed-behavior ERC-20.** The `StakedWood` slashing path uses end-of-function bulk burns; if WOOD is ever migrated to a token with transfer hooks (ERC-777 / 1363) the slashing path becomes reentrancy-exposed. Cross-referenced with the "delegatecall-only-to-BatchExecutorLib" assumption above.
