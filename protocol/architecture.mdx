...
Holds the protocol-level fee parameters — the management and performance **splits** (`mgmtSplit` / `perfSplit`) plus the protocol and guardian fee recipients — shared by every per-vault governor. There is no `protocolFeeBps` or `guardianFeeBps`: each party's cut is a leg of the split. Values are read only at propose time and snapshotted into the proposal, never read live at settle, so changing a fee never affects an in-flight proposal. It is a plain `Ownable2Step` contract behind the protocol multisig.
