...
**Wiring:** `factory` and `swood` are stamped at `initialize()`. Governors are not stamped at init — they are appended to the authorized set via `addGovernor(gov)` (factory or registry owner) as each syndicate is created. (sWOOD's reverse handle to the registry is set once post-deploy via `setRegistry`, since the registry is deployed after sWOOD.)
