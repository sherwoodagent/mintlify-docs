...
- `setAgentFeeBps(bps)` — **owner-only**. Sets the vault's agent performance fee (the cut a proposing agent earns on profit). Defaults to 5% (500 bps) at creation; reverts above the vault cap of 50% (5000 bps). The governor reads `agentFeeBps()` live from the vault at settlement and additionally clamps it to `maxPerformanceFeeBps`. Replaces the old per-proposal fee argument.
...
| `_agentFeeBps` | uint256 | agent performance fee for this vault (basis points; default 500, max 5000; owner-set via `setAgentFeeBps`, read live by the governor at settlement) |
...
**Key functions:**
- `propose(vault, strategy, metadataURI, strategyDuration, executeCalls, settlementCalls, coProposers)` — create proposal with separate opening/closing call arrays. There is no per-proposal fee argument — the agent fee is the vault's `agentFeeBps`, read live from the vault at settlement. `strategy` is the address whose `positionValue()` the vault polls for live NAV during the strategy window; pass `address(0)` to opt out of live NAV (queue-only). The proposal's `snapshotTimestamp = block.timestamp - 1` (closes a flash-delegate window on 2s L2 blocks). `reviewEnd` and `executeBy` are stamped in the same tx based on the current `reviewPeriod` and `executionWindow`.
...
UUPS upgradeable contract (owned by the Sherwood protocol multisig) coordinating the guardian review layer. **The registry holds zero WOOD** — guardian stake, vault-owner bonds, DPoS delegation, vote checkpoints, and slashing execution all live on the `StakedWood` (sWOOD) contract (V1.5 sWOOD-staking split, PRs #353/#355). The registry reads vote weight from sWOOD and *calls* sWOOD to slash; it owns the review accounting, reward pools, and the slash-appeal reserve. Interacts with `SyndicateGovernor` + `SyndicateFactory` via privileged hooks.
...
- Multi-asset guardian-fee reward pools (`fundProposalGuardianPool`, `claimProposalReward`, `claimDelegatorProposalReward`) + W-1 escrow for blocked transfers
...
- Block-side reward attribution: on a review that resolves blocked, the registry emits `BlockerAttributed(proposalId, epochId, blocker, weight)` per blocker. WOOD epoch block-rewards are distributed **off-chain via Merkl** — its bot reads these events plus delegation / commission events to build Merkle roots, and guardians claim at merkl.xyz. There is no on-chain `fundEpoch` / `claimEpochReward` / `sweepUnclaimed` / `minter` (all removed in V1.5).
