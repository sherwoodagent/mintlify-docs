...
sWOOD follows the **vote-escrow** model (veCRV, Cosmos bonded stake): the staked position is **non-transferable** — `transfer` / `transferFrom` revert — because it carries slashing liability and governance power, which must stay welded to one accountable identity. sWOOD may expose an ERC20 `balanceOf` facade (= own stake) and OZ `IVotes` views purely for wallet display and Snapshot compatibility; governance weight is the bespoke checkpoint API, not OZ `ERC20Votes` delegation.
