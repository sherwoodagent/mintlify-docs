...
```mermaid
graph TD
    F["SyndicateFactory (UUPS)"] -->|deploys| V["SyndicateVault (ERC1967 Proxy)"]
    F -->|"deploys per-vault governor (BeaconProxy)"| G["SyndicateGovernor (BeaconProxy, one per vault)"]
    GB["GovernorBeacon (UpgradeableBeacon)"] -.->|implementation| G
    PC["ProtocolConfig (Ownable2Step)"] -.->|fee params snapshotted at propose| G
    F -->|binds owner bond on create| SW["StakedWood / sWOOD (UUPS)"]
    G -->|guardian review + slashing hooks| R["GuardianRegistry (UUPS, shared)"]
    ...
    V -.->|"reads governor via governorOf(vault)"| F
```

The vault is the identity — all DeFi positions (Moonwell supply/borrow, Uniswap swaps, Aerodrome LP) live on the vault address. Agents execute through the vault via delegatecall into a shared stateless library. **Each vault has its own governor** — the factory deploys a `BeaconProxy` governor per vault at `createSyndicate`, bound to that vault at initialize; the governor manages that one vault's proposal lifecycle, voting, and settlement. All governor proxies read their implementation from the shared `GovernorBeacon` (owned by the protocol multisig), so a protocol-wide governor upgrade is a single `beacon.upgradeTo(newImpl)`. The vault resolves its governor address via `factory.governorOf(vault)` — there is no governor storage on the vault itself.
...
- `transferPerformanceFee(token, to, amount)` — governor-only fee distribution after settlement. Has no amount / recipient / token validation on the vault side — the governor is trusted to transfer only what settlement math dictates, to a pre-vetted recipient (proposer, co-proposer, vault owner, or `protocolFeeRecipient`), in the proposal's `asset()`. This is a deliberate trust assumption: the governor is beacon-upgradeable by the protocol multisig (via the shared `GovernorBeacon`) — same trust root as the old UUPS path, different mechanism — so compromising it compromises the fee path by construction. See [Trust Assumptions](#trust-assumptions) below.
...
- `redemptionsLocked()` — pull-model lock: resolves the vault's governor via `factory.governorOf(address(this))` and reads `governor.getActiveProposal() != 0` live on every call (the view is no-arg — each governor serves exactly one vault). No `lockRedemptions()` / `unlockRedemptions()` state-flipping functions exist on the vault; the lock is derived, not stored.
...
Proposal lifecycle, voting, execution, settlement, and collaborative proposals — **one governor per vault**. Each governor is a `BeaconProxy` deployed by the factory at `createSyndicate` and bound to its vault at initialize; `governor.vault()` returns it, and the scoped views are no-arg (`getActiveProposal()`, `openProposalCount()`, `getCooldownEnd()`). The governor is **not UUPS and not Ownable** — upgrades flow through the shared `GovernorBeacon`, and parameter authority belongs to the vault owner (resolved live via `vault.owner()`). Inherits `GovernorParameters` (abstract) for vault-owner parameter setters (frozen while any proposal is open) and `GovernorEmergency` (abstract) for the four-way emergency-settle split introduced in PR #229. The old vault-registration surface (`addVault` as registration, `removeVault`, `isRegisteredVault`, `getRegisteredVaults`) is gone — the factory's `governorOf(vault)` mapping is the only wiring.
...
**Protocol + guardian fees (ProtocolConfig snapshot):** `protocolFeeBps` / `protocolFeeRecipient` / `guardianFeeBps` / `guardiansFeeRecipient` live on the global `ProtocolConfig` contract — fee setters no longer exist on the governor. At propose time the governor **snapshots all four fields into the proposal**; settlement charges what voters saw, so a later config change cannot reprice an in-flight proposal. Max protocol fee is 10% (1000 bps), max guardian fee 5% (500 bps); a nonzero fee requires its recipient to be set first (coupled both directions).
...
**Storage (per-vault — each governor holds state for exactly one vault):**
| Slot | Type | Description |
|------|------|-------------|
| `vault` | address | the one vault this governor serves (set at initialize; lives on `GovernorParameters`) |
| `protocolConfig` | address | `ProtocolConfig` reference — read at propose time for fee snapshots (lives on `GovernorParameters`) |
| `factory` | address | authorized factory (can call `forceSetParams` / `setProtocolConfig`; lives on `GovernorParameters`) |
| `_params` | struct | this vault's `GovernorParams` (voting/execution windows, veto, fee caps, durations, collaboration) |
| `_proposals` | mapping | proposal ID → `StrategyProposal` struct — includes the four `snapshot*Fee*` fields captured from `ProtocolConfig` at propose |
...
| `_activeProposal` | uint256 | current live proposal (scalar — one vault, one active proposal; read via no-arg `getActiveProposal()`) |
...
| `__gap` | uint256[] | reserved storage for future upgrades |

### GovernorParameters

Abstract contract inherited by SyndicateGovernor. Contains all governance constants, parameter setters, and validation helpers. Since the per-vault governor redesign it also holds the governor's wiring slots (`vault`, `protocolConfig`, `factory`) and the vault's own `GovernorParams` struct.

**Vault-owner setters, frozen during proposals:** Each setter is `onlyVaultOwner` (resolved live via `vault.owner()`), validates bounds at call time, applies the change immediately, and emits a uniform `ParameterChangeFinalized(paramKey, oldValue, newValue)` event — but every setter is gated behind `whenNoActiveProposal` and reverts with `ParamsFrozenDuringProposal` while any proposal is open (Draft included), so parameters cannot shift under an in-flight proposal. Setters: `setVotingPeriod`, `setExecutionWindow`, `setVetoThresholdBps`, `setMaxPerformanceFeeBps`, `setMinStrategyDuration`, `setMaxStrategyDuration`, `setCooldownPeriod`, `setCollaborationWindow`, `setMaxCoProposers`. Fee setters (`setProtocolFeeBps` etc.) no longer exist on the governor — protocol-level fees live on [ProtocolConfig](#protocolconfig).

**Rescue path:** `factory.setParamsOverride(vault, params)` (protocol-multisig-only) drives the governor's factory-only `forceSetParams` — it bypasses the proposal freeze but **not** the bounds validation.

**Parameter bounds (hardcoded):**

| Parameter | Bounds |
|-----------|--------|
| Voting period | 24h - 30d (`MIN_VOTING_PERIOD = 24 hours`) |
| Execution window | 1h - 7d |
| Veto threshold (bps) | 20% - 50% (`MIN_VETO_THRESHOLD_BPS = 2000`) |
...
The hard floors on voting period (24h) and veto threshold (20%) stop a vault owner from configuring a governor whose proposals are effectively un-vetoable.

### GovernorBeacon

`UpgradeableBeacon` owned by the protocol multisig. Every per-vault governor is a `BeaconProxy` that reads its implementation from this beacon at call time, so a protocol-wide governor upgrade is a single `beacon.upgradeTo(newImpl)` — no per-vault migration. The factory stores the beacon address (`factory.beacon`, owner-settable via `setBeacon` for a future beacon migration; already-deployed proxies keep reading the original beacon live).

### ProtocolConfig

Plain (non-upgradeable) `Ownable2Step` global contract holding the protocol-level fee parameters shared by all per-vault governors: `protocolFeeBps` (max 10% / 1000 bps), `protocolFeeRecipient`, `guardianFeeBps` (max 5% / 500 bps), `guardiansFeeRecipient`. Recipient coupling is enforced both directions — a nonzero fee requires its recipient to be set, and a recipient cannot be zeroed while its fee is nonzero.

The governor reads it **only at propose time**, snapshotting all four fields into the proposal — settlement charges what voters saw, never the live config. If the contract ever needs replacement, governors accept a new address via the factory-only `setProtocolConfig(address)`; snapshotting means no in-flight proposal is affected.

<Note>
**`ABSOLUTE_MAX_STRATEGY_DURATION` raised 30 days → 3650 days (~10y).** This `public constant` is the ceiling that `setMaxStrategyDuration` validates against; raising it enables **indefinitely-lived strategies** (e.g. [Leveraged Aerodrome CL](/protocol/strategies/leveraged-aerodrome-cl), which keeps one long-duration proposal open and does routine management proposal-free). Because it is a compiled constant, fresh deployments pick it up immediately, while an **already-deployed governor** built from older code keeps the 30-day ceiling (`setMaxStrategyDuration(> 30 days)` reverts) until a governor UUPS upgrade (new impl + `upgradeToAndCall`). The constant is global to the single governor, so once raised any vault owner can set a multi-year `maxStrategyDuration` — the blast radius is acknowledged and a **per-vault duration cap is the recommended mainnet hardening (deferred)**. Note the owner emergency exits (`unstick` / `emergencySettleWithCalls`) are duration-gated, so a multi-year proposal keeps them dormant for that long.
</Note>
...
- Reward attribution — **both** guardian reward streams are distributed **off-chain via Merkl**, in WOOD, claimed at merkl.xyz:
  ...
  - *Guardian performance fees* (revenue): at settlement the governor routes the fee slice to the `guardiansFeeRecipient` multisig and emits `GuardianFeeAccrued`. The team swaps it to WOOD and airdrops approvers/delegators weekly. The bot reconstructs the DPoS split from `GuardianFeeAccrued` + the registry's `getApproverWeights(governor, pid)` + sWOOD `getPast*` checkpoints (commission at `settledAt`, delegation at `openedAt`). `GuardianFeeAccrued` is emitted **only on actual fee delivery** — an escrowed transfer emits `FeeTransferFailed` instead, so the bot never attributes an undelivered fee.
...
**Wiring:** `factory` and `swood` are stamped at `initialize()`. Governors are not stamped at init — they are appended to the authorized set via `addGovernor(gov)` (factory or registry owner) as each fund is created. (sWOOD's reverse handle to the registry is set once post-deploy via `setRegistry`, since the registry is deployed after sWOOD.)
...
UUPS upgradeable factory. Deploys vault proxies (ERC1967) **and a dedicated `BeaconProxy` governor per vault** (initialized against the vault, the shared `GuardianRegistry`, and `ProtocolConfig` in the same `createSyndicate` tx), records the pairing in the `governorOf(vault)` mapping, authorizes the new governor with the registry via `addGovernor`, and optionally registers ENS subnames. There is **no shared governor** — vaults resolve their governor through `factory.governorOf(vault)`. Verifies ERC-8004 identity on creation (skipped when registries are `address(0)`, as on Robinhood testnet, which has no identity/ENS registry yet).
...
**Config setters (owner-only):** `setVaultImpl`, `setCreationFee`, `setManagementFeeBps`, `setUpgradesEnabled`, `setEnsRegistrar`, `setGuardianRegistry`, `setBeacon` (re-points the beacon **new** governor proxies read from — a routine governor-impl upgrade is `beacon.upgradeTo(newImpl)`, not this), `setProtocolConfig` (existing governors keep the reference set at their init), and `setParamsOverride(vault, params)` — the protocol-multisig rescue path that force-sets a vault's governor params, bypassing the proposal freeze but not the bounds. There is no `setGovernor` and no shared governor slot — each vault's governor is deployed at `createSyndicate` and recorded in `governorOf(vault)`; a governor-logic swap goes through the beacon.
...
| `beacon` | address | `GovernorBeacon` that new per-vault governor proxies read their implementation from |
| `protocolConfig` | address | `ProtocolConfig` passed to each new governor at initialize |
| `_governorOf` | mapping | vault → its dedicated governor (read via `governorOf(vault)`; vaults resolve their governor through this) |
...
1. **`vault.transferPerformanceFee(token, to, amount)` has no amount / recipient / token cap.** The governor (beacon-upgradeable by protocol multisig via `GovernorBeacon`) is trusted not to pass arbitrary values here. A compromised governor implementation could transfer any ERC-20 the vault holds to any address — and a single `beacon.upgradeTo` swaps the logic under every vault's governor at once. Mitigation is governance-level: the beacon owner is a multisig with its own delay, not an instant EOA. Tracked as item **A38** in the pre-mainnet punch list.
...
<Warning>
All three core contracts are upgradeable — Vault and Factory via UUPS, the per-vault Governor via the shared `GovernorBeacon` (storage-layout rules apply identically: every governor proxy shares one implementation). Both the vault and governor include `__gap` arrays for upgrade safety. Violating storage layout rules will corrupt contract state and may be irreversible. Follow these rules strictly when modifying any upgradeable contract.
</Warning>
