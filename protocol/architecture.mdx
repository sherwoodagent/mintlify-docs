...
description: "Smart contract design — vault, per-vault governor, factory, withdrawal queue, and the guardian layer"
icon: "cubes"
---
...
`totalAssets()` is vault float minus the queue reserve minus escrowed unpaid fees — there is no strategy NAV term. While a strategy is executing, instant `redeem` / `withdraw` return 0 and depositors use `requestRedeem` on the [withdrawal queue](/protocol/vault-liquidity). Instant `deposit` is closed for the whole open-proposal window. At settlement the vault stamps one frozen post-fee price; anyone can `claim`.
...
At settlement the governor charges the [two fees](/protocol/governance/economics) — a management fee on assets × time deployed, charged on every settlement whether the fund gained or lost, and a performance fee on value above the fund's previous peak price per share, charged only on profitable settlements — using rates snapshotted at propose time. Each is divided once by the splits held on `ProtocolConfig`. Failed fee transfers escrow rather than bricking settlement. Parameter setters are owner-instant but frozen while any proposal is open, so the owner is expected to be a multisig; there is no separate onchain timelock. The governor also carries the four owner-only [emergency-settlement](/protocol/governance/settlement) paths for stuck strategies.
...
<Note>
The in-repo `WoodToken.sol` is a non-production test fixture, not the live token. Mainnet WOOD is an external ERC-20 launched on Robinhood Chain and tradeable on Uniswap. Bind to the mainnet address `0xF8BC08092C06dB6148114DCf82AF881F1085f92b` (see [Deployments](/reference/deployments#wood)), not the in-repo artifact.
</Note>
...
Holds the protocol-level fee splits — how the management fee divides across agent, protocol and guardians (default 60/20/20) and how the performance fee divides across agent, protocol, guardians and the fund owner (default 50/15/25/10) — plus the `protocolFeeRecipient` and `guardiansFeeRecipient` addresses, shared by every per-vault governor. Each split must sum to exactly 10,000 bps. Values are read only at propose time and snapshotted into the proposal, never read live at settle, so changing a split never affects an in-flight proposal. It is a plain `Ownable2Step` contract behind the protocol multisig.
...
Holds a weighted basket of tokenized stocks and rebalances toward target weights. Rebalance sizing reads a Chainlink push feed bound per basket slot, and swaps route through a pluggable swap adapter rather than a hardcoded venue, so the strategy is DEX-agnostic. On the active deployment swaps route through Synthra (a Uniswap-V3-compatible DEX) via the `UniswapSwapAdapter` / `SynthraSwapAdapter`. See [Portfolio Strategy](/protocol/strategies/uniswap).
