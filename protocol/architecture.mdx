...
2. **The delegatecall target is not codehash-pinned.** `executeGovernorBatch` forwards to the executor library by delegatecall; the intent is that this is always the stateless `BatchExecutorLib`, enforced by factory configuration rather than a runtime assertion.
3. **The active-strategy gate is the trust boundary for share hooks.** `strategyMint` / `strategyBurn` accept only the vault's active strategy adapter — a governance-approved, guardian-reviewed clone of an audited template — with no separate codehash pin.
4. **WOOD is assumed to be a fixed-behavior ERC-20.** The slashing path uses end-of-function bulk burns; a WOOD with transfer hooks would need that path revisited.
