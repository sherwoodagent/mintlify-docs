...
**One governor per syndicate:** Every syndicate gets its own governor at creation — the factory deploys a `BeaconProxy` governor bound to the new vault at initialize. Proposals are scoped to that vault by construction (`governor.vault()` returns it), and only shareholders of that vault vote on its proposals. The vault owner tunes their governor's parameters (voting period, veto threshold, cooldown, etc.) — changes are frozen while any proposal is open. All governor proxies read their implementation from a shared `GovernorBeacon`, so a protocol-wide governor upgrade is a single `beacon.upgradeTo(newImpl)` by the protocol multisig.
...
struct StrategyProposal {
    ...
    address strategy;              // live-NAV source polled via positionValue(); address(0) to opt out
    ...
    address snapshotProtocolFeeRecipient;   // │ ProtocolConfig fields snapshotted at propose time —
    uint256 snapshotGuardianFeeBps;         // │ settlement charges what voters saw; a later config
    address snapshotGuardiansFeeRecipient;  // ┘ change cannot reprice an in-flight proposal
}
```
...
| votingPeriod | Vault owner (per-vault governor setter) | How long voting lasts. Hard floor `MIN_VOTING_PERIOD = 24 hours` |
| reviewPeriod | GuardianRegistry (protocol-owner setter) | Guardian review window after voting ends (single global value) |
| executionWindow | Vault owner (per-vault governor setter) | Time after guardian review ends to execute |
| vetoThresholdBps | Vault owner (per-vault governor setter) | Min AGAINST votes (% of total shares) to reject a proposal. Bounded to `MIN_VETO_THRESHOLD_BPS = 2000` (20%) — `MAX_VETO_THRESHOLD_BPS = 5000` (50%). The floor stops an owner from making their vault trivially veto-proof; values above 50% would let a minority of FOR votes block any rejection regardless of opposition |
| maxPerformanceFeeBps | Vault owner (per-vault governor setter) | Cap on agent fees |
| maxStrategyDuration | Vault owner (per-vault governor setter) | Cap on how long a strategy can run |
| cooldownPeriod | Vault owner (per-vault governor setter) | Withdrawal window between strategies |

<Note>
**Governance parameters are per-syndicate, managed by the vault owner.** Each vault's governor carries its own `GovernorParams`. Setters (`setVotingPeriod`, `setExecutionWindow`, `setVetoThresholdBps`, `setMaxPerformanceFeeBps`, `setMin`/`MaxStrategyDuration`, `setCooldownPeriod`, `setCollaborationWindow`, `setMaxCoProposers`) are callable by the **vault owner** (resolved live via `vault.owner()`), validate hard bounds at call time, and apply immediately — but revert with `ParamsFrozenDuringProposal` while any proposal is open (Draft included), so parameters cannot shift under an in-flight proposal. The protocol multisig has a rescue path: `factory.setParamsOverride(vault, params)` bypasses the freeze but **not** the bounds. Protocol-level fee parameters are separate — they live on `ProtocolConfig`, not the governor (see [Economics](/protocol/governance/economics#protocol-fee)).
</Note>
