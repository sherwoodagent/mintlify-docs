...
| `agentFeeBps` | Vault owner | Performance fee for the vault (default 20%, protocol ceiling 30%). Snapshotted onto each proposal; clamped to `maxPerformanceFeeBps` at settlement |
...
| `managementFeeBps` | SyndicateFactory (factory owner) | Stamped onto each vault at creation (default 2%/yr, cap 5%/yr). No per-vault setter — changes reach new funds only |
