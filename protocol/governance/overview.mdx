...
<Steps>
  ...
  <Step title="The strategy settles">
    Once the strategy duration ends, anyone can trigger settlement. The vault replays the pre-committed unwind calls, P&L is booked, the management fee and (on profit above the high-water mark) the performance fee are charged and split, and a `ProposalSettled` event is emitted.
  </Step>
  <Step title="Cooldown">
...
- **Fee snapshot** — the agent performance fee and the protocol's management / performance splits are all snapshotted from live config at propose time. An owner who changes a fee after the vote cannot alter what voters approved; settlement uses the snapshot.
...
| `agentFeeBps` | Vault owner | Performance fee for the vault (default 20%, vault cap 25%). Snapshotted onto each proposal; clamped to `maxPerformanceFeeBps` at settlement |
...
| `mgmtSplit`, `perfSplit` (protocol / guardian / owner shares) | ProtocolConfig (protocol multisig) | Global; snapshotted into the proposal at propose |
