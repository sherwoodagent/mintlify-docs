...
A proposal that needs an uncertified target does not name it in those batches. `proposeWithSandbox` stores a third, frozen payload — funding, arbitrary calls, declared tokens — that executes through a per-proposal [Call sandbox](/protocol/governance/sandbox). Guardians review that payload the same way they review the batches.
