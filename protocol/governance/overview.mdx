...
| agentFeeBps | Vault owner | Performance fee for the vault (default 5%, max 50%); not a proposal field. Read live at settlement, clamped to `maxPerformanceFeeBps` |
...
<Note>
**Governance parameters are global, not per-syndicate.** A single `GovernorParameters` instance (inherited by the governor) serves every registered vault. Changing a parameter changes it for every vault at once. Setters are owner-instant (no on-chain timelock) — the owner is a multisig that enforces its own delay externally (e.g. Gnosis Safe + Zodiac Delay). There is no per-vault override.
</Note>
