...
Four fees are distributed from strategy profits at settlement:

| Fee | Recipient | Set by | Purpose |
|-----|-----------|--------|---------|
| Protocol fee | `protocolFeeRecipient` | Protocol multisig via `ProtocolConfig` (snapshotted onto each proposal at propose) | Protocol revenue |
| Guardian fee | `guardiansFeeRecipient` (team multisig; redistributed as WOOD via Merkl) | Protocol multisig via `ProtocolConfig` (snapshotted onto each proposal at propose) | Reward guardian review work |
...
- `protocolFeeBps` — percentage of gross profit taken as protocol fee (max 10% / 1000 bps, `MAX_PROTOCOL_FEE_BPS` on `ProtocolConfig`)
- `protocolFeeRecipient` — address that receives the protocol fee
- Both live on **`ProtocolConfig`** — a plain `Ownable2Step` global contract shared by every per-vault governor. Fee setters no longer exist on the governor.
- **Snapshotted at propose time.** The governor reads `ProtocolConfig` only when a proposal is created, copying all four fee fields (`protocolFeeBps`, `protocolFeeRecipient`, `guardianFeeBps`, `guardiansFeeRecipient`) onto the proposal. Settlement charges the snapshot — what voters saw — so a later config change can never reprice an in-flight proposal.
- `ProtocolConfig` setters are owner-instant and emit `ParameterChangeFinalized(paramKey, old, new)`; the owner is a multisig that enforces its own delay externally (e.g. Gnosis Safe + Zodiac Delay). Setting a nonzero `protocolFeeBps` requires `protocolFeeRecipient` to be set first (and the recipient cannot be zeroed while the fee is nonzero).

### Guardian Fee

A slice of gross profit (max 5% / 500 bps) routed at settlement to the `guardiansFeeRecipient` multisig, then swapped to WOOD and airdropped to approving guardians and their delegators weekly via Merkl. Like the protocol fee, `guardianFeeBps` / `guardiansFeeRecipient` live on `ProtocolConfig` and are snapshotted onto each proposal at propose time.

**`GuardianFeeAccrued` is emitted only on actual fee delivery.** If the transfer to the recipient fails (e.g. blacklist), the amount goes to the standard unclaimed-fee escrow and `FeeTransferFailed` is emitted instead — the off-chain Merkl bot never attributes a fee that was not delivered, avoiding a double-pay when the escrow is later recovered.
...
- `agentFeeBps` lives on the **vault**, not the proposal. Set it with `vault.setAgentFeeBps(bps)` (owner only) or the CLI `sherwood fund set-agent-fee --bps <bps>`.
...
Fees are calculated and distributed in a strict order — protocol and guardian fees first (from gross profit), then agent fee, then management fee:

```
profit = balanceAfter - capitalSnapshot
if profit > 0:
  // protocol + guardian fees use the ProtocolConfig snapshot taken at propose
  protocolFee   = profit * proposal.snapshotProtocolFeeBps / 10000
  guardianFee   = profit * proposal.snapshotGuardianFeeBps / 10000
  ...
  transfer protocolFee to snapshotProtocolFeeRecipient   (try/catch → escrow on fail)
  transfer guardianFee to snapshotGuardiansFeeRecipient  (try/catch → escrow on fail;
                                                          GuardianFeeAccrued emitted only if delivered)
...
The protocol and guardian fees are calculated on gross profit. Agent and management fees are calculated on the net profit (after protocol and guardian fees). Management fee is calculated on the remaining profit after the agent's cut. This ensures combined fees never exceed profit and maintains a clear priority order.
...
- Each vault's governor tracks a scalar `_activeProposal` — the currently executing proposal ID (0 if none), read via the no-arg `getActiveProposal()`
- `executeProposal` reverts if `getActiveProposal() != 0`
