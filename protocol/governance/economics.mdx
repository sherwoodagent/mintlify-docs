...
- Defaults to **5% (500 bps)** at vault creation. Capped at **50% (5000 bps)** by the vault.
- The governor reads `agentFeeBps()` **live from the vault at settlement** (it is not snapshotted at propose time) and additionally clamps it to `maxPerformanceFeeBps`. A change therefore takes effect for any proposal that settles after it.
...
if profit > 0:
  ...
  agentFeeBps   = min(vault.agentFeeBps(), maxPerformanceFeeBps)   // read live from the vault, clamped to the governor cap
...
**Safety:** the vault's `agentFeeBps` is capped at 5000 bps (50%) by the vault and additionally clamped to `maxPerformanceFeeBps` (governor parameter) at settlement. `protocolFeeBps` is capped at 1000 bps (10%). `managementFeeBps` is capped at 1000 bps (10%) and set at the factory level.
...
- Vault owner sets one `agentFeeBps` for the vault (default 5%); every proposal inherits it
- Read live from the vault at settlement, clamped to `maxPerformanceFeeBps`
