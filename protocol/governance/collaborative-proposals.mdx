...
propose(
    vault,                // target vault
    strategy,             // live-NAV source (positionValue()); address(0) to opt out
    metadataURI,
    strategyDuration,
    executeCalls,
    settlementCalls,
...
5. **Maximum co-proposers:** the governor enforces a hard ceiling of `ABSOLUTE_MAX_CO_PROPOSERS = 10`, and the runtime `maxCoProposers` parameter (bounds 1–10) gates the currently-allowed value — default is `5` at launch. Since governors are per-vault, the **vault owner** can raise their vault's cap up to 10 via `setMaxCoProposers` (applies immediately, but reverts with `ParamsFrozenDuringProposal` while any proposal is open); above 10 would require a governor implementation upgrade through the shared beacon. (Lead + `maxCoProposers` = total recipients at settlement.)
