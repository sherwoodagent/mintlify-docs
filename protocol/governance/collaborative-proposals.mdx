...
5. **Maximum co-proposers:** the governor enforces a hard ceiling of `ABSOLUTE_MAX_CO_PROPOSERS = 10`, and the runtime `maxCoProposers` parameter (owner-instant setter, bounds 1–10) gates the currently-allowed value — deployed value is `5` at launch. The deployed cap can be raised up to 10 via `setMaxCoProposers` (owner-instant; the multisig enforces its own delay); above 10 would require a governor upgrade. (Lead + `maxCoProposers` = total recipients at settlement.)
