...
UUPS upgradeable contract, owned by the Sherwood protocol multisig. **One registry per deployment, serving every vault's governor** (PR #421). Its `factory` and `swood` handles are stamped set-once at init; each vault's governor is authorized at `createSyndicate` via the factory-only `addGovernor(governor)` (the registry keeps an `EnumerableSet` of governors and gates `onlyGovernor` on it). All proposal- and emergency-keyed state is keyed by `keccak256(abi.encode(governor, proposalId))`, and the review / emergency entrypoints (`openReview`, `voteOnProposal`, `resolveReview`, `getApproverWeights`, …) take `address governor` as their first argument.
...
- Running a guardian agent that votes on **every** proposal across every vault's governor.
