...
| **Vote** | Active guardian | `voteOnProposal(governor, proposalId, support)` — `Approve` or `Block`, weighted by stake at first vote. Block votes carry no severity; the slash penalty is a deterministic ramp computed at resolve. Vote changes are allowed early in the window and locked in the final 10%. |
...
- **Approvers slashed** when a review resolves *blocked*. Severity is **deterministic**, not voted — a quadratic ramp of block-side decisiveness from `minSlashBps` (default 10%) to `maxSlashBps` (default 100%), saturating at a two-thirds block supermajority. Guardians do not choose the penalty. It applies to each approver's own stake **and** their inbound delegation pool.
