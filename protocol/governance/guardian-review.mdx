...
- **A slice of every fee the protocol charges.** The guardian network takes a share of *both* legs of the fee model — at the launch splits, **20% of every management fee and 25% of every performance fee** (`ProtocolConfig.mgmtSplit` / `perfSplit`), routed onchain to the guardian fee recipient at settlement. Every fund on the protocol pays the same way, and review is funded when markets go flat as well as when they run. The split across the guardians who approved that strategy is **by parked stake weight** (`GuardianRegistry.getApproverWeights`, the vote-time `_voteStake` snapshot), not by underwritten coverage — the payout bot's sole attribution signal is the governor's `GuardianFeeAccrued` event, whose spec names `getApproverWeights` as the per-proposal split. A coverage-based split (`getApproverCoverage`, which returns the ledger's settled allocations and a `priced` flag) exists on-chain and is the intended future basis, since parked weight can pay an approver whose fully-committed bond underwrote nothing; until the bot migrates, weight is what pays.
...
| **Vote** | Active guardian | `voteOnProposal(governor, proposalId, support, lockWood)` — `Approve` or `Block`, weighted by the guardian's own stake at `snapshotAt` (growth-gated `getPastStake`). `lockWood` is the WOOD an approver declares as coverage (ignored on a Block; see [Coverage](/protocol/governance/coverage)). Block votes carry no severity; the slash penalty is a deterministic ramp computed at resolve. Vote changes are allowed early in the window and locked in the final 10%. |
...
<Warning>
**A review nobody opens is not a review that gets skipped — it is a proposal that executes unreviewed.** `resolveReview` returns `blocked = false` unconditionally for a review that was never opened, whatever the calldata was, and `voteOnProposal` reverts `ReviewNotOpen` until `openReview` lands. So blocking is **two transactions**: `openReview(governor, proposalId)` from `voteEnd`, then the vote. Both are permissionless to call, and a guardian that watches only for *opened* reviews structurally cannot be the thing that opens them.
</Warning>
