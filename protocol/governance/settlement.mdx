...
Two fees are charged at settlement, in this order, using the splits snapshotted at propose:

```
pnl = balanceAtSettle − capitalSnapshot

# 1. management fee — on time, not on profit
mgmtFee = assetSeconds × managementFeeBps / (10_000 × 365 days)   // rate cap 3%
        → agent / protocol / guardian, per the snapshotted mgmtSplit

# 2. performance fee — only above the fund's previous peak
base    = min(vault.aboveHighWaterMark(), max(pnl, 0))            // read after the mgmt fee
perfFee = base × min(agentFeeBps, maxPerformanceFeeBps)           // agent rate cap 25%
        → agent / protocol / guardian / owner, per the snapshotted perfSplit
vault.ratchetHighWaterMark()

# remainder stays in the vault, accruing to all depositors
```

There is no separate `protocolFeeBps` or `guardianFeeBps`: the protocol and guardian shares are **legs of the split** on each of those two fees, and a leg whose recipient is unconfigured folds into the agent's remainder. The guardian leg routes onchain to a fee-recipient multisig, then distributes to individual guardians off-chain, weekly, via Merkl. See [Economics](/protocol/governance/economics) for the full breakdown.
