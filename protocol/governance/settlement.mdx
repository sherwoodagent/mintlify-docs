...
3. Governor records the proposal ID in its scalar `_activeProposal` slot (each governor serves exactly one vault) — this is what `vault.redemptionsLocked()` reads
...
<Note>
**Pull-model redemption lock — no `lockRedemptions()` function.** There is no separate state-flipping call on the vault. Instead, the vault exposes `redemptionsLocked()` which resolves its governor via `factory.governorOf(vault)` and reads the no-arg `governor.getActiveProposal() != 0` live every time. Setting `_activeProposal` inside `executeProposal` is what flips the lock; clearing it inside `_finishSettlement` is what unlocks it. This removes a whole class of state-desync bugs where the vault could disagree with the governor about whether a strategy is live.
</Note>
...
| **`emergencySettleWithCalls`** | Vault owner | After `strategyDuration` ends | Stores owner-provided custom calls in the registry (`openEmergency`) and opens a guardian review window | Owner's bond must cover `requiredOwnerBond(vault)` at call time; reverts otherwise |
...
| **`finalizeEmergencySettle`** | Vault owner | After `reviewEnd` | Takes `proposalId` only; the registry returns the stored calls | Reverts (and slashes owner bond) if guardian block quorum reached; otherwise executes via `vault.executeGovernorBatch` and transitions to Settled |
...
<Tabs>
  ...
  <Tab title="emergencySettleWithCalls (guardian-reviewed custom calls)">
    ...
    **Flow:**
    1. Owner calls `emergencySettleWithCalls(proposalId, calls)` with the custom call array. The governor reverts with `OwnerBondInsufficient` if `registry.ownerStake(vault) < registry.requiredOwnerBond(vault)`. Assuming the bond check passes, the governor calls `registry.openEmergency(proposalId, keccak256(abi.encode(calls)), calls)` — the **full calls array is stored in the registry** alongside its hash — and emits `EmergencySettleProposed`. **Calls do not execute yet.**
    ...
    3. After `reviewEnd`, the owner calls `finalizeEmergencySettle(proposalId)` — **no calldata re-submission**. The governor calls `registry.finalizeEmergency(proposalId)`, which returns `(blocked, calls)` from registry storage:
        - If the block quorum was reached (`blocked == true`) → owner's bond is slashed (WOOD burned) and `finalizeEmergencySettle` reverts. The owner must fall back to `unstick`, post a fresh bond, or propose different calls.
        - If not blocked → governor runs `vault.executeGovernorBatch(calls)` with the registry-returned calls and transitions the proposal to Settled. Fees distributed normally.
...
Settle:
  ...
  Path 3 — Emergency settle with custom calls (owner-bonded, guardian-reviewed):
    1. Owner calls emergencySettleWithCalls(proposalId, calls) — bond rechecked, full calls stored in registry via openEmergency
    2. 24h guardian review window; guardians can block; owner can cancel
    3. Owner calls finalizeEmergencySettle(proposalId) after reviewEnd (proposalId only; registry returns stored calls)
    4. If guardians blocked: owner bond slashed, function reverts
    5. Otherwise: vault executes the registry-returned calls via executeGovernorBatch, same P&L math

  All paths end with:
    - _activeProposal cleared (redemptions unlock automatically)
    - Cooldown starts
