...
- **Outbound** (initiateReturn → HC drain): `returnsInitiated` is set to `true` _before_ `_drainHC()` queues HC actions. The adapter checks this flag and returns `(0, false)` → Lane B during that window, preventing new depositors from locking into a settling proposal.
