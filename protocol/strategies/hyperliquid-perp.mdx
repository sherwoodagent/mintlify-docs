...
- `sweepToVault()` is permissionless and accepts any positive balance — a strategy that loses money on HyperCore must still be able to return whatever USDC arrives back to the vault (no settlement floor that would permanently strand funds on lossy outcomes)
