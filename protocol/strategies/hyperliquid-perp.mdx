...
The `HyperliquidPerpStrategy` lets a fund run leveraged perpetual positions on Hyperliquid from an on-chain vault. USDC is pulled from the vault, **bridged EVM → HyperCore spot** via Circle's CoreDepositWallet (`HyperliquidBridge.bridgeUsdcToSpot`), and then moved **spot → perp margin** via `L1Write.sendUsdClassTransfer`. The proposer drives trading actions (open long, open short, stop-loss, close, plus multi-asset variants) through `updateParams`. Position state is read live from HyperCore via `L1Read.position2()` — nothing is cached on-chain.
...
graph TD
    ...
    subgraph "PriceRouter (Lane A — disabled pending audit)"
        S -->|"positions() → HL_PERP"| C5["HyperliquidPerpAdapter"]
        C5 -->|"L1Read.accountMarginSummary"| C6["HyperCore equity"]
        C6 -->|"haircut + instantCap check"| V
    end
...
The strategy reports one position of kind `HL_PERP` pointing at the `HyperliquidPerpAdapter`. The vault **never trusts a self-reported value** — the governance-owned `PriceRouter` prices the position by calling `L1Read.accountMarginSummary` (the AccountMarginSummary precompile), applying a realizability haircut and an instant-size cap.

**Lane A is currently disabled** (`laneAEnabled[HL_PERP] = false`) pending a post-audit multisig call. Until enabled, the strategy is Lane B only: deposits and redeems during an active proposal go through the `VaultWithdrawalQueue` and settle at the frozen per-proposal price. See [Deposits & Withdrawals](/protocol/vault-liquidity).

**Transit guards** — two windows where HC equity is temporarily unreliable:
- **Inbound** (execute → HC credit, ~1–2 blocks): fresh clone has zero HC equity → adapter returns `(0, true)` → PriceRouter G3 forces Lane B. No code required in the strategy.
- **Outbound** (initiateReturn → HC drain): `_drainHC()` queues HC actions, then `returnsInitiated` is set to `true` in the same transaction. The adapter checks this flag and returns `(0, false)` → Lane B during that window, preventing new depositors from locking into a settling proposal.
