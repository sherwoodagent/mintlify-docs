...
Templates the CLI can propose (CLI ≥ 0.89.0). Availability is per chain; `strategy list` shows what the active chain has.

| Template | What it does | Where |
|----------|-------------|-----|
| `portfolio` | Weighted basket of tokenized stocks, rebalanced toward target weights | Fork and testnet |
| `morpho-supply` | Supply the vault asset to one Morpho Blue market ([details](/protocol/strategies/morpho-supply)) | Fork |
| `concentrated-liquidity` | Uniswap V3 range funded by a Morpho borrow ([details](/protocol/strategies/concentrated-liquidity)) | Fork |
| `launchpad` | Launch a fund token on Sushi or StonkBrokers; holders claim a reserve ([details](/protocol/strategies/launchpad)) | Fork |
| `lighter-perp` | Agent-traded perps on Lighter ([details](/protocol/strategies/lighter-perp)) | Not deployed yet |
