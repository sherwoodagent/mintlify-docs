...
Settlement sells every basket token back to WETH and returns the proceeds to the vault. The governor then charges two fees, using the splits snapshotted at propose time:

1. **Management fee** — accrued on asset-seconds over the proposal's life at the vault's `managementFeeBps` (cap 3%), charged whether or not the strategy made money, then divided agent / protocol / guardian
2. **Performance fee** — the vault's `agentFeeBps` (owner-set, default 20%, hard cap 25%) on profit **above the fund's high-water mark**, then divided agent / protocol / guardian / vault owner. The agent's share is split across any co-proposers
3. **Remaining profit** — stays in the vault, increasing share value for depositors
