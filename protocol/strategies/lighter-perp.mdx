...
<Note>**In development — not yet live.** The `LighterPerpStrategy` is not yet available on Sherwood's current deployment. Lighter runs on **Robinhood mainnet (chain 4663)**, collateralized in USDG; the template also accepts the [Robinhood-mainnet fork (chain 9994663)](/reference/deployments#robinhood-mainnet-fork-chain-9994663) used for pre-launch testing and refuses every other chain at deploy time.</Note>
...
<Info>
  **What the custody boundary does cover.** The agent holds only an L2 API key. That key can **trade** — place and cancel orders — and nothing else. It cannot withdraw, cannot transfer, and cannot change keys: every one of those is authenticated by the venue to the account's registered owner (the strategy contract), and the contract only ever pushes USDG to its own vault. The contract can rotate or revoke the agent's key at any time, and can cancel, force-close, and drain the account without the agent's cooperation.
</Info>
...
Anything that arrives at the clone after the settlement stamp — a late-maturing withdrawal, a second `queueWithdraw` that drains what the first one missed — is **residue**. The clone reports it to the vault through a delivery interface (`hasUndeliveredValue`, `undeliveredValue`, `hasUnvaluedResidue`) and the vault collects it through a single measured door, `collectResidue`, so that depositors who exited in the meantime still receive their share. Anyone can claim a matured withdrawal into the clone (`recoverResiduals`), but only the vault can move USDG from the clone into the vault.

<Note>That vault-side delivery and residue surface is **not in the v1 vault** — `collectResidue` and the `IStrategyDelivery` hooks ship with this strategy, not ahead of it. It is part of what is still in development here.</Note>
